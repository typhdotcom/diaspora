...
### Lightlike Dressing (`LightlikeDressing.lean`)

Single triangles are lightlike: E = p = 1/3, invariant mass zero, velocity c. Pairing with an opposite-orientation cycle creates timelike composites.

| Configuration | E | p | m² | v |
| :--- | :--- | :--- | :--- | :--- |
| Bare triangle | 1/3 | 1/3 | 0 | 1 |
| Dressed pair (triangle + n-cycle) | 1/3 + 1/n | \|1/3 - 1/n\| | 4/(3n) | < 1 |
| Symmetric pair (triangle + anti-triangle) | 2/3 | 0 | 4/9 | 0 |

* `bare_triangle_lightlike`: Single triangles have m² = 0.
* `bare_triangle_has_momentum`: p > 0; bare triangles have no rest frame.
* `dressed_pair_timelike`: Triangle + opposite-orientation cycle has m² > 0.
* `dressed_mass_sq_formula`: m² = 4/(3n) for triangle dressed by n-cycle.
* `rest_frame_implies_dressed`: Rest frame requires dressing.
* `symmetric_pair_at_rest`: Equal dressing (n = 3) gives v = 0.
* `dressed_subluminal`: All dressed pairs have v < 1.
* `larger_dressing_smaller_mass`: Larger dressing cycle means smaller invariant mass.

Interpretation: A bare triangle travels at c. Pairing with an opposite-orientation cycle creates invariant mass and subluminal velocity. Symmetric pairing (triangle + anti-triangle) produces a system at rest with m = 2/3.
